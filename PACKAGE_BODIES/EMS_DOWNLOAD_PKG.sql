--------------------------------------------------------
--  DDL for Package Body EMS_DOWNLOAD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "INSTITUTE"."EMS_DOWNLOAD_PKG" as
...
procedure fees_invoice (p_stud_id in number, p_inst in number, p_acyr_id in number)
AS
...
procedure result_print (p_test_id in number, p_std_id in number, p_inst in number, p_acyr_id in number)
IS
    INM VARCHAR2(50);
    IADD VARCHAR2(500);
    ICITY VARCHAR2(50);
