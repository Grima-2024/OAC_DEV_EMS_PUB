--------------------------------------------------------
--  DDL for Package Body UPLOAD_FILE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "INSTITUTE"."UPLOAD_FILE_PKG" as 
    
    --(SELECT INSTUTUTE_ID FROM USERS_MST WHERE UPPER(USERNAME) = UPPER(V('APP_USER')))
    --V('APP_ACADEMIC')
--------------------------------------------------UPLOAD ATTENDANCE----------------------------------------------------------
procedure get_attendance_to_collection(P_FILE IN VARCHAR2) as
Begin
    APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION(
        ...
        for i in (with lst1 as (select * from apex_application_temp_files where name = P_FILE)
            SELECT line_number, col001, col002, col003, col004, col005, col006, col007, col008, col009, col010, col011, col012, col013, col014, col015,
                UPLOAD_FILE_PKG.GET_STUD_ID(col001, col002, col003) AS STUD_ID,
                UPLOAD_FILE_PKG.GET_MED_ID(col007) AS MED_ID,
                UPLOAD_FILE_PKG.GET_STD_ID(col008,UPLOAD_FILE_PKG.GET_MED_ID(col007)) AS STD_ID,
                UPLOAD_FILE_PKG.GET_SEM_ID(col009,GET_STD_ID(col008,GET_MED_ID(col007))) AS SEM_ID,
                UPLOAD_FILE_PKG.GET_DIV_ID(col010,UPLOAD_FILE_PKG.GET_STD_ID(col008,GET_MED_ID(col007))) AS DIV_ID,
                UPLOAD_FILE_PKG.GET_SUB_ID(col011,UPLOAD_FILE_PKG.GET_STD_ID(col008,GET_MED_ID(col007))) AS SUB_ID,
                UPLOAD_FILE_PKG.GET_TIME_ID(col012,col013) as TIME_ID
                FROM lst1, TABLE(
    ...
    APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION(
        ...
        for i in (with file1 as (select * from apex_application_temp_files where name = P_FILE)
            SELECT line_number, col001, col002, col003, col004, col005, col006, col007, col008, col009, col010, col011, col012, col013,
                ...
                UPLOAD_FILE_PKG.GET_MED_ID(col041) AS MED_ID,
                UPLOAD_FILE_PKG.GET_STD_ID(col042, UPLOAD_FILE_PKG.GET_MED_ID(col041)) AS STD_ID,
                UPLOAD_FILE_PKG.GET_SEM_ID(col043,GET_STD_ID(col042,GET_MED_ID(col041))) AS SEM_ID,
                UPLOAD_FILE_PKG.GET_DIV_ID(col044,GET_STD_ID(col042,GET_MED_ID(col041))) AS DIV_ID,
                UPLOAD_FILE_PKG.GET_STATE_ID(col033,UPLOAD_FILE_PKG.GET_COUNTRY_ID(col032)) AS STATE_ID,
                UPLOAD_FILE_PKG.GET_COUNTRY_ID(col032) AS COUNTRY_ID,
                UPLOAD_FILE_PKG.GET_STATE_ID(col038,UPLOAD_FILE_PKG.GET_COUNTRY_ID(col037)) AS TEMP_STATE_ID,
                UPLOAD_FILE_PKG.GET_COUNTRY_ID(col037) AS TEMP_COUNTRY_ID
                FROM file1, TABLE(
                        APEX_DATA_PARSER.parse(
     ...
     APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION(
        ...
        for i in (with QB as (select * from apex_application_temp_files where name = P_FILE)
            SELECT line_number, col001, col002, col003, col004, col005, col006, col007, col008, col009, col010, col011, col012, col013,
                    ...
                    UPLOAD_FILE_PKG.GET_MED_ID(col012) AS MED_ID,
                    UPLOAD_FILE_PKG.GET_STD_ID(col013, UPLOAD_FILE_PKG.GET_MED_ID(col012)) AS STD_ID,
                    UPLOAD_FILE_PKG.GET_SEM_ID(col014,GET_STD_ID(col013,GET_MED_ID(col012))) AS SEM_ID,
                    UPLOAD_FILE_PKG.GET_SUB_ID(col015,GET_STD_ID(col013,GET_MED_ID(col012))) AS SUB_ID
                FROM QB, TABLE(
                        APEX_DATA_PARSER.parse(
...
BEGIN
    ...
    for i in (with lst1 as (select * from apex_application_temp_files where name = P_FILE)
        SELECT line_number, col001, col002, col003, col004, col005, col006, col007,
            UPLOAD_FILE_PKG.GET_MED_ID(col001) AS MED_ID,
            UPLOAD_FILE_PKG.GET_STD_ID(col002, UPLOAD_FILE_PKG.GET_MED_ID(col001)) AS STD_ID,
            UPLOAD_FILE_PKG.GET_SEM_ID(col003, UPLOAD_FILE_PKG.GET_STD_ID(col002,UPLOAD_FILE_PKG.GET_MED_ID(col001))) AS SEM_ID
        FROM lst1, TABLE(
            APEX_DATA_PARSER.parse(
                p_content                     => lst1.blob_content,
                p_add_headers_row             => 'Y',
...
BEGIN
    ...
    for i in (with lst1 as (select * from apex_application_temp_files where name = P_FILE)
        SELECT line_number, col001, col002, col003, col004, col005, col006, col007, col008, col009, col010,
            ...
            UPLOAD_FILE_PKG.GET_MED_ID(col002) AS MED_ID,
            UPLOAD_FILE_PKG.GET_STD_ID(col003,UPLOAD_FILE_PKG.GET_MED_ID(col002)) AS STD_ID,
            UPLOAD_FILE_PKG.GET_SEM_ID(col004,GET_STD_ID(col003,GET_MED_ID(col002))) AS SEM_ID,
            UPLOAD_FILE_PKG.GET_DIV_ID(col005,UPLOAD_FILE_PKG.GET_STD_ID(col003,GET_MED_ID(col002))) AS DIV_ID,
            UPLOAD_FILE_PKG.GET_SUB_ID(col006,UPLOAD_FILE_PKG.GET_STD_ID(col003,GET_MED_ID(col002))) AS SUB_ID
        FROM lst1, TABLE(
            APEX_DATA_PARSER.parse(
                p_content                     => lst1.blob_content,
                p_add_headers_row             => 'Y',
...
IS
    stud_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_STUD_ID'; 
...
BEGIN  
    ...
    WHERE UPPER(FIRSTNAME) = UPPER(p_stud_f)
        ...
        AND INSTITUTE_ID = (SELECT INSTUTUTE_ID FROM USERS_MST WHERE UPPER(USERNAME) = UPPER(V('APP_USER')))
...
IS
    medium_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_MED_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    stand_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_STD_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    semt_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_SEM_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    divi_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_DIV_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    subj_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_SUB_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    time_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_TIME_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    count_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_COUNTRY_ID'; 
    l_params logger.tab_param; 
BEGIN  
...
IS
    st_id NUMBER;
    l_scope logger_logs.scope%type := 'UPLOAD_FILE_PKG.GET_STATE_ID'; 
    l_params logger.tab_param; 
BEGIN  
